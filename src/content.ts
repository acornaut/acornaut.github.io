...
function frontmatter(raw: string, fileName: string): { data: Frontmatter; body: string } {
  const match = raw.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/)
  if (!match) throw new Error(`${fileName} needs a frontmatter block.`)

  const data = Object.fromEntries(match[1]
    .split(/\r?\n/)
    .filter(Boolean)
    .map((line) => {
      const separator = line.indexOf(':')
      if (separator === -1) throw new Error(`${fileName} has an invalid frontmatter line: ${line}`)
...
function toProject(fileName: string, raw: string): Project {
  ...
  const required = ['kind', 'title', 'summary', 'status', 'year', 'tags'] as const
  required.forEach((field) => {
    if (!data[field]) throw new Error(`${fileName} is missing '${field}'.`)
  })

  if (data.kind !== 'Development' && data.kind !== 'Writing') throw new Error(`${fileName} has an invalid kind.`)
  if (!['In progress', 'Complete', 'Experiment', 'Draft'].includes(data.status)) throw new Error(`${fileName} has an invalid status.`)
  ...
  return {
    ...
    updates: data.updates?.split('|').map((update) => update.trim()).filter(Boolean),
  }
}
...
export const projects = Object.entries(projectFiles)
  ...
  .sort((first, second) => second.year.localeCompare(first.year) || first.title.localeCompare(second.title))

export const stack = ['Java', 'Hytale API', 'TypeScript', 'React', 'C#', '.NET', 'SQL', 'Git']
